...
from emuleo.http import SSG, SSR
...
class App:
    """
    App class to create an emuleo app
        ...
        if self.type == "ssr":
            ...
            for page in self.pages.rglob("*.html"):
                if "index.html" in str(page.name):
                    ...
                    pages[
                        "/"
                        + str(page.relative_to(self.pages).as_posix()).replace(
                            ".html", ""
                        )
                    ] = page
            server = {}
            for file in self.server.rglob("*.py"):
                if "index.py" in str(file.name):
                    ...
                    server[
                        "/"
                        + str(file.relative_to(self.server).as_posix()).replace(
                            ".py", ""
                        )
                    ] = file
            static = {}
    ...
    def run(
        ...
        if self.type == "ssr" and isinstance(self.http, SSR):
            ...
            print(
                "[#F43F5E bold]❌ Run method is only available for SSR instances[/#F43F5E bold]"
            )
            exit(0)
    ...
    def build(self, dest: Path) -> None:
        ...
        if self.type == "ssg" and isinstance(self.http, SSG):
            ...
            print(
                "[#F43F5E bold]❌ Run method is only available for SSG instances[/#F43F5E bold]"
            )
            exit(0)
...
class Response:
    """
    Response class to create an emuleo response
