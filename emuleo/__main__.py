...
from emuleo import __version__
from emuleo.utils.cli import fetch_template
...
def options_selector_using(options: list[str], prompt: str, console: Console) -> str:
    ...
    selected_option = Prompt.ask(
        prompt, choices=[str(i + 1) for i in range(len(options))]
    )
    console.print(
...
def main() -> None:
    """
    Emuleo is a CLI tool to scaffold your emuleo projects.
    """
    pass
...
def init(_path: str | Path) -> None:
    """
    Initialize a new emuleo project.
